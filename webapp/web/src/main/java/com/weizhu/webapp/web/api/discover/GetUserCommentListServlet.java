...
@Singleton
public class GetUserCommentListServlet extends HttpServlet {
	...
	@Inject
	public GetUserCommentListServlet(Provider<RequestHead> requestHeadProvider, DiscoverV2Service discoverV2Service,
	...
	public void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		GetUserCommentListRequest.Builder requestBuilder = GetUserCommentListRequest.newBuilder()
				.setUserId(userId)
				.setSize(size);
		...
		GetUserCommentListResponse response = Futures.getUnchecked(discoverV2Service.getUserCommentList(head, requestBuilder.build()));
		...
		JsonArray userCommentArray = new JsonArray();
		for (DiscoverV2Protos.ItemComment itemComment : response.getItemCommentList()) {
			userCommentArray.add(DiscoverUtil.itemCommentJson(itemComment, userMap, teamMap, positionMap, imageUrlPrefix));
		}
		JsonArray refItemArray = new JsonArray();
		for (DiscoverV2Protos.Item item : response.getRefItemList()) {
			refItemArray.add(DiscoverUtil.itemJson(item, userMap, teamMap, positionMap, imageUrlPrefix));
		}
		
		JsonObject result = new JsonObject();
		result.add("item_comment", userCommentArray);
		...
		result.add("ref_item", refItemArray);
