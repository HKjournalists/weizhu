...
@Singleton
public class LearnItemServlet extends HttpServlet {
	...
	@Inject
	public LearnItemServlet(Provider<RequestHead> requestHeadProvider, DiscoverV2Service discoverV2Service) {
	...
	public void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		final int learnDuration = ParamUtil.getInt(httpRequest, "learn_duration", 0); // 学习时长，单位秒 
		
		final RequestHead head = requestHeadProvider.get();
		
		LearnItemResponse response = Futures.getUnchecked(discoverV2Service.learnItem(head, LearnItemRequest.newBuilder()
				.setItemId(itemId)
