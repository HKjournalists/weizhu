...
_138=$.extend({},opts.queryParams,_138||{},{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
var _13a=null;
if(_137!=ul){
var node=$(ul).prev();
...
var _230=$.extend({},opts.queryParams,{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
...
$.extend(opts,_523||{});
var _524=$.extend({},opts.queryParams,{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
...
_52b.bind("load",function(e){
	var matchs = e.target.contentDocument.title.match(/.*Error.*(\d{3}).*/)||[];
	var states = matchs[1];
	if(!!window.Wz){
		if(states == 403){
			$.messager.alert('错误','您没有改操作权限！','error');
...
var _755=$.extend({},opts.queryParams,{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
if(opts.pagination){
	if(!!opts.changePages){
		opts.changePages(_755,{page:opts.pageNumber||1,rows:opts.pageSize});
	}else{
...
var _8d2=$.extend({},opts.queryParams,{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
if(opts.pagination){
...
var cc=_8ed.children("td").children("div");
cc.hide();
var _8ef=$.extend({},opts.queryParams||{},{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
...
_91f=$.extend({},opts.queryParams,{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
_91f.id=id;
}
if(_91f.id){
...
_9fc=$.extend({},opts.queryParams,_9fc||{},{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)});
...
if(opts.mode=="remote"){
grid.datagrid("clearSelections");
grid.datagrid("load",$.extend({},opts.queryParams,{q:q},{_t:new Date().getTime(),company_id: (window.Wz&&window.Wz.cur_company_id)}));
}else{
if(!q){
return;
}
...
var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	insertRow: function(target, index, row){
		...
		function _insert(frozen){
			...
			var tr = v.find('>.datagrid-body>.datagrid-btable>tbody>tr[datagrid-row-index='+index+']').first();
			if(frozen){
				tr = v.find('tr[datagrid-row-index='+index+']').first();
			}
	...
	updateRow: function(target, rowIndex, row){
		...
		if (this.canUpdateDetail){
			...
			opts.onExpandRow.call(target, rowIndex, row)
		}
	},
