...
public class UpdateDiscoverBannerOrderServlet extends HttpServlet {
	
	private final Provider<AdminHead> adminHeadProvider;
	private final AdminDiscoverService adminDiscoverService;

	@Inject
	public UpdateDiscoverBannerOrderServlet(Provider<AdminHead> adminHeadProvider, AdminDiscoverService adminDiscoverService) {
	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws ServletException, IOException {
		// 1. 取出参数
		List<Integer> bannerOrderIdList = ParamUtil.getIntList(httpRequest, "banner_order_str", Collections.<Integer>emptyList());
		...
		AdminDiscoverProtos.SetDiscoverHomeRequest request = AdminDiscoverProtos.SetDiscoverHomeRequest.newBuilder()
				.addAllBannerOrderId(bannerOrderIdList)
				.build();
