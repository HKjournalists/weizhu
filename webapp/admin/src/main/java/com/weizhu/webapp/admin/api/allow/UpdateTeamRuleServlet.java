...
@Singleton
public class UpdateTeamRuleServlet extends HttpServlet {
	...
	@Inject
	public UpdateTeamRuleServlet(Provider<AdminHead> adminHeadProvider, AllowService allowService) {
	...
	public void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		try {
			List<String> teamIdStrList = DBUtil.COMMA_SPLITTER.splitToList(teamIdStr);
		...
		UpdateTeamRuleRequest updateTeamRuleRequest = UpdateTeamRuleRequest.newBuilder()
				...
				.build();
		UpdateTeamRuleResponse updateTeamRuleResponse = Futures.getUnchecked(allowService.updateTeamRule(head, updateTeamRuleRequest));
		
		httpResponse.setContentType("application/json;charset=UTF-8");
		JsonUtil.PROTOBUF_JSON_FORMAT.print(updateTeamRuleResponse, httpResponse.getWriter());
	}
