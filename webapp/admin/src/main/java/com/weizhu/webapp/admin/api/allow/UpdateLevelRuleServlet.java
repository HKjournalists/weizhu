...
public class UpdateLevelRuleServlet extends HttpServlet {
	...
	@Inject
	public UpdateLevelRuleServlet(Provider<AdminHead> adminHeadProvider, AllowService allowService) {
	...
	public void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		final String levelIdStr = ParamUtil.getString(httpRequest, "level_id", "");
		List<Integer> levelIdList = new ArrayList<Integer>();
		try {
			List<String> levelIdStrLIst = DBUtil.COMMA_SPLITTER.splitToList(levelIdStr);
			for (String levelId : levelIdStrLIst) {
				levelIdList.add(Integer.parseInt(levelId));
			}
		...
		UpdateLevelRuleRequest updateLevelRuleRequest = UpdateLevelRuleRequest.newBuilder()
				.setModelId(modelId)
				.setRuleId(ruleId)
				...
				.build();
		UpdateLevelRuleResponse updateLevelRuleResponse = Futures.getUnchecked(allowService.updateLevelRule(head, updateLevelRuleRequest));
		
		httpResponse.setContentType("application/json;charset=UTF-8");
		JsonUtil.PROTOBUF_JSON_FORMAT.print(updateLevelRuleResponse, httpResponse.getWriter());
	}
