...
public class ExportDiscoverItemLikeServlet extends HttpServlet {
	...
	@Inject
	public ExportDiscoverItemLikeServlet(Provider<AdminHead> adminHeadProvider, AdminDiscoverService adminDiscoverService, AdminUserService adminUserService) {
	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws ServletException, IOException {
		...
		final SXSSFWorkbook wb = new SXSSFWorkbook(new XSSFWorkbook(Resources.getResource("com/weizhu/webapp/admin/api/discover/export_discover_item_like_file.xlsx").openStream()));
		try {
			...
			while (true) {
				AdminDiscoverProtos.GetItemLikeListResponse response = Futures.getUnchecked(
						this.adminDiscoverService.getItemLikeList(head, AdminDiscoverProtos.GetItemLikeListRequest.newBuilder()
								.setItemId(itemId)
								.setStart(start)
								.setLength(length)
								.build()));
				...
				for (DiscoverV2Protos.ItemLike itemLike : response.getItemLikeList()) {
					...
					row.createCell(0).setCellValue(itemLike.getItemId());
					row.createCell(1).setCellValue(itemLike.getUserId());
					
					UserProtos.User user = refUserMap.get(itemLike.getUserId());
					if (user != null) {
						...
						row.createCell(2).setCellValue("已删除用户:" + itemLike.getUserId());
					}
					
					row.createCell(9).setCellValue(df.format(new Date(itemLike.getLikeTime() * 1000L)));
			...
			httpResponse.setHeader("Content-Disposition", "attachment;filename=export_discover_item_like.xlsx");
