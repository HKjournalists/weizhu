...
@Singleton
public class AssignedSalerServlet extends HttpServlet {
	...
	@Inject
	public AssignedSalerServlet(Provider<AdminHead> adminHeadProvider, ToolsProductclockService toolsProductclockService) {
	...
	public void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
		...
		final long salerId = ParamUtil.getLong(httpRequest, "saler_id", 0L);
		
		final AdminHead adminHead = adminHeadProvider.get();
		
		AssignedSalerResponse response = Futures.getUnchecked(toolsProductclockService.assignedSaler(adminHead, AssignedSalerRequest.newBuilder()
