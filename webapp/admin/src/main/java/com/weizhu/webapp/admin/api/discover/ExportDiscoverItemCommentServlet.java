...
public class ExportDiscoverItemCommentServlet extends HttpServlet {
	...
	@Inject
	public ExportDiscoverItemCommentServlet(Provider<AdminHead> adminHeadProvider, AdminDiscoverService adminDiscoverService, AdminUserService adminUserService) {
	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws ServletException, IOException {
		...
		final SXSSFWorkbook wb = new SXSSFWorkbook(new XSSFWorkbook(Resources.getResource("com/weizhu/webapp/admin/api/discover/export_discover_item_comment_file.xlsx").openStream()));
		try {
			// 导出条目的评论人员列表
			...
			while (true) {
				AdminDiscoverProtos.GetItemCommentListResponse response = Futures.getUnchecked(
						this.adminDiscoverService.getItemCommentList(
								head, AdminDiscoverProtos.GetItemCommentListRequest.newBuilder()
								.setItemId(itemId)
								.setStart(start)
								.setLength(length)
								.build()));
				...
				for (DiscoverV2Protos.ItemComment itemComment : response.getItemCommentList()) {
					...
					row.createCell(0).setCellValue(itemComment.getCommentId());
					row.createCell(1).setCellValue(itemComment.getItemId());
					row.createCell(2).setCellValue(itemComment.getUserId());
					
					UserProtos.User user = refUserMap.get(itemComment.getUserId());
					if (user != null) {
						row.createCell(3).setCellValue(user.getBase().getUserName());
						...
						if (user.getTeamCount() > 0) {
							...
							for (int i=0; i<5 && teamIt.hasNext(); ++i) {
								row.createCell(4 + i).setCellValue(teamIt.next().getTeamName());
							}
						...
						row.createCell(3).setCellValue("已删除用户:" + itemComment.getUserId());
					}
					
					row.createCell(10).setCellValue(df.format(new Date(itemComment.getCommentTime() * 1000L)));
					row.createCell(11).setCellValue(itemComment.getCommentText());
			...
			httpResponse.setHeader("Content-Disposition", "attachment;filename=export_discover_item_comment_file.xlsx");
