...
public class ExportDiscoverItemScoreServlet extends HttpServlet {
	...
	@Inject
	public ExportDiscoverItemScoreServlet(Provider<AdminHead> adminHeadProvider, AdminDiscoverService adminDiscoverService, AdminUserService adminUserService) {
	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws ServletException, IOException {
		...
		final SXSSFWorkbook wb = new SXSSFWorkbook(new XSSFWorkbook(Resources.getResource("com/weizhu/webapp/admin/api/discover/export_discover_item_score_file.xlsx").openStream()));
		try {
			...
			while (true) {
				AdminDiscoverProtos.GetItemScoreListResponse response = Futures.getUnchecked(
						this.adminDiscoverService.getItemScoreList(head, AdminDiscoverProtos.GetItemScoreListRequest.newBuilder()
								.setItemId(itemId)
								.setStart(start)
								.setLength(length)
								.build()));
				...
				for (DiscoverV2Protos.ItemScore itemScore : response.getItemScoreList()) {
					...
					row.createCell(0).setCellValue(itemScore.getItemId());
					row.createCell(1).setCellValue(itemScore.getUserId());

					UserProtos.User user = refUserMap.get(itemScore.getUserId());
					if (user != null) {
						...
						row.createCell(2).setCellValue("已删除用户:" + itemScore.getUserId());
					}
					
					row.createCell(9).setCellValue(df.format(new Date(itemScore.getScoreTime() * 1000L)));
					row.createCell(10).setCellValue(itemScore.getScoreNumber());
			...
			httpResponse.setHeader("Content-Disposition", "attachment;filename=export_discover_item_score.xlsx");
