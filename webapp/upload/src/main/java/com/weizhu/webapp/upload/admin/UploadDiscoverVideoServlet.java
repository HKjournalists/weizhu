...
public class UploadDiscoverVideoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws ServletException, IOException {
		...
		Video video = this.getVideoInfo(head, tmpFile, md5, size);
		if (video == null) {
			JsonObject obj = new JsonObject();
			obj.addProperty("result", "FAIL_UNKNOWN");
	...
	private Video getVideoInfo(AdminHead head, File videoFile, String md5, int size) {
		...
		String name = head.getCompanyId() + "/discover/video/" + md5 + "." + type;
