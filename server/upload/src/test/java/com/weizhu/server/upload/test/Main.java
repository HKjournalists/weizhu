package com.weizhu.server.upload.test;
...
import com.weizhu.server.upload.Main.ServerModule;

public class Main {

	public static void main(String[] args) throws Throwable {
		if (System.getProperty("logback.configurationFile") == null) {
			System.setProperty("logback.configurationFile", "com/weizhu/server/upload/test/logback.xml");
		}
		if (args.length <= 0) {
			final Injector injector = Guice.createInjector(
					Stage.DEVELOPMENT, 
					new ConfModule(Resources.getResource("com/weizhu/server/upload/test/server.conf")), 
					new ServerModule()
					);
